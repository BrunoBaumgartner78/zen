...
import { Application, Graphics, Sprite, Texture, Circle, Rectangle } from 'pixi.js'
...
export default function PixiGardenPlain() {
  ...
  useEffect(() => {
    ...
    sandRef.current = sand; grainRef.current = grain; vignetteRef.current = vignette; pathsRef.current = pathsG
    grain.eventMode = 'none'; vignette.eventMode = 'none'
    ...
    const onKey = (ev: KeyboardEvent) => {
      ...
      if (mod && key === 's') { ev.preventDefault(); exportPNG(); return }
      if (key === 'm') { ev.preventDefault(); setMenuOpen(v => !v); return }
    ...
    // Autoplay-Policy: erster Pointer startet Audio
  ...
  // Repaints
  useEffect(() => { repaintSand(); repaintVignette(); soundRef.current?.applyTheme(theme) }, [theme])
  useEffect(() => { repaintPaths(paths, brush) }, [paths, brush])
  useEffect(() => {
    const on = () => { repaintSand(); repaintGrain(true); repaintVignette(); repaintPaths(paths, brush) }
      ...
      if (!g) {
        ...
        g.on('pointerdown', (e: any) => {
          ...
          const onMove = (ev: any) => {
            ...
            setItems(prev => prev.map(s => s.id === it.id ? { ...s, x: origin.x + dx, y: origin.y + dy } : s))
          }
          const onUp = () => {
            g!.cursor = 'grab'
  ...
  function repaintSand() {
    const app = appRef.current, sand = sandRef.current
    if (!app || !sand) return
    sand.clear()
  ...
  function repaintVignette() {
    const app = appRef.current, v = vignetteRef.current
    if (!app || !v) return
  ...
  function repaintPaths(all: Point[][], width = 12) {
    ...
    const groove = (path: Point[]) => {
      ...
      path.forEach((p, i) => { const x = p.x + shadowOffset, y = p.y + shadowOffset; if (i === 0) g.moveTo(x, y); else g.lineTo(x, y) })
      g.lineStyle({ width: width * 0.5, color: 0xC9C3B5, alpha: 0.9, cap: 'round', join: 'round' })
      path.forEach((p, i) => { if (i === 0) g.moveTo(p.x, p.y); else g.lineTo(p.x, p.y) })
      g.lineStyle({ width: width * 0.35, color: 0xFFFFFF, alpha: 0.25, cap: 'round', join: 'round' })
      path.forEach((p, i) => { const x = p.x + highlightOffset, y = p.y + highlightOffset; if (i === 0) g.moveTo(x, y); else g.lineTo(x, y) })
    }
    all.forEach(groove)
  }
  ...
  function drawItem(g: Graphics, it: Item, selected: boolean) {
    ...
    switch (it.kind) {
      ...
      case 'leaf': {
        ...
        g.beginFill(0x000000, 0.10); g.drawEllipse(it.x + 4, it.y + 7, w * 0.6, h * 0.55); g.endFill()
        ...
        g.moveTo(it.x - w*0.2, it.y); g.quadraticCurveTo(it.x, it.y - h*0.1, it.x + w*0.25, it.y - h*0.2)
        if (selected) outline(g, it.x, it.y, w * 0.7, h * 0.7)
        break
      }
      case 'lantern': {
        const w = it.w ?? 44, h = it.h ?? 64
        const x = it.x, y = it.y
        g.beginFill(0x000000, 0.12); g.drawEllipse(x + 6, y + h*0.15, w*0.8, h*0.25); g.endFill()
      ...
      case 'rake': {
        const w = it.w ?? 58, h = it.h ?? 10
        const x = it.x, y = it.y
        g.beginFill(0x000000, 0.10); g.drawEllipse(x + 5, y + 4, w * 0.6, h * 1.2); g.endFill()
        g.lineStyle({ width: 4, color: 0x8B6E49, alpha: 1 })
        g.moveTo(x - w/2, y); g.lineTo(x + w/2 - 12, y)
  ...
  function setHitArea(g: Graphics, it: Item) {
    switch (it.kind) {
      ...
      case 'waveRing':
        ...
        break
      case 'leaf':
        g.hitArea = new Rectangle(it.x - (it.w ?? 48)/2, it.y - (it.h ?? 26)/2, (it.w ?? 48), (it.h ?? 26))
        break
      case 'lantern':
        g.hitArea = new Rectangle(it.x - (it.w ?? 42)/2, it.y - (it.h ?? 60)/2, (it.w ?? 42), (it.h ?? 60))
        break
      case 'rake':
  ...
  function addItem(kind: ItemKind) {
    ...
    switch (kind) {
      ...
      case 'leaf':       base.w = 52; base.h = 28; base.angle = (Math.random()-0.5)*0.6; break
      case 'lantern':    base.w = 44; base.h = 64; break
      case 'rake':       base.w = 58; base.h = 10; base.angle = (Math.random()-0.5)*0.3; break
      case 'waveRing':   base.r = 34; break
    }
    setItems(s => { const next = [...s, base]; queueMicrotask(pushHistory); return next })
  ...
  function removeSelected() {
    ...
    setItems(s => { const next = s.filter(i => i.id !== selectedId); queueMicrotask(pushHistory); return next })
    setSelectedId(null)
    soundRef.current?.clickUi()
  }

  function clearRidges() { setPaths([]); pushHistory(); soundRef.current?.clickUi() }
  ...
  function loadScene() {
    ...
    try {
      ...
      setTheme(d.theme); setBrush(d.brush); setPaths(d.paths); setItems(d.items); setSelectedId(d.selectedId ?? null)
  ...
  function undo() {
    const u = undoRef.current; if (u.length <= 1) return
    const curr = u.pop()!; redoRef.current.push(curr)
    const prev = structuredClone(u[u.length - 1])
    setTheme(prev.theme); setBrush(prev.brush); setPaths(prev.paths); setItems(prev.items); setSelectedId(prev.selectedId)
    soundRef.current?.clickUi()
  }
  function redo() {
    ...
    setTheme(next.theme); setBrush(next.brush); setPaths(next.paths); setItems(next.items); setSelectedId(next.selectedId)
    soundRef.current?.clickUi()
  }
  ...
  const hbBtn: React.CSSProperties = {
    width: 40, height: 40, borderRadius: 12, border: '1px solid rgba(0,0,0,0.15)',
    background: 'rgba(255,255,255,0.65)', backdropFilter: 'blur(8px)', cursor: 'pointer',
    display: 'grid', placeItems: 'center', boxShadow: '0 6px 18px rgba(0,0,0,0.14)'
  }
  const barCommon: React.CSSProperties = { width: 20, height: 2, background: '#111', borderRadius: 2, transition: 'transform .25s ease, opacity .25s ease' }
  const barTop: React.CSSProperties = { ...barCommon, transform: menuOpen ? 'translateY(6px) rotate(45deg)' : 'translateY(-4px) rotate(0deg)' }
  const barMid: React.CSSProperties = { ...barCommon, opacity: menuOpen ? 0 : 1 }
  const barBot: React.CSSProperties = { ...barCommon, transform: menuOpen ? 'translateY(-6px) rotate(-45deg)' : 'translateY(4px) rotate(0deg)' }

  const dockBase: React.CSSProperties = {
    position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)',
    width: 280, zIndex: 10, background: 'rgba(255,255,255,0.6)', backdropFilter: 'blur(8px)',
    borderRadius: 16, boxShadow: '0 10px 28px rgba(0,0,0,0.16)', padding: 14, fontFamily: 'system-ui, sans-serif',
    transition: 'transform .28s ease, opacity .28s ease', opacity: 1, willChange: 'transform'
  }
  const dockStyle: React.CSSProperties = menuOpen
    ? { ...dockBase, transform: 'translateY(-50%) translateX(0)', opacity: 1, pointerEvents: 'auto' }
    : { ...dockBase, transform: 'translateY(-50%) translateX(-320px)', opacity: 0, pointerEvents: 'none' }

  const titleStyle: React.CSSProperties = { fontSize: 13, fontWeight: 800, letterSpacing: 0.8, textTransform: 'uppercase', margin: 0, marginBottom: 8 }
  const small: React.CSSProperties = { fontSize: 12 }
  const row: React.CSSProperties = { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }
  const btn: React.CSSProperties = { padding: '6px 10px', borderRadius: 10, border: '1px solid rgba(0,0,0,0.15)', cursor: 'pointer', background: 'white' }
    ...
    <div style={rootStyle}>
      ...
      <div style={hbWrap}>
        <button
          ...
          onClick={() => { setMenuOpen(v => !v); soundRef.current?.clickUi() }}
          style={hbBtn}
        >
      ...
      <div style={dockStyle}>
        ...
        <label style={{ display: 'block', ...small }}>
          Harke-Größe
          <input type="range" min={4} max={40} value={brush} onChange={(e)=>setBrush(Number(e.target.value))} style={{ width: '100%' }} />
        </label>

        <div style={row}>
          <button onClick={() => addItem('stoneFlat')} style={btnDark}>Stein flach</button>
          <button onClick={() => addItem('stoneOval')} style={btnDark}>Stein oval</button>
          <button onClick={() => addItem('stoneTall')} style={btnDark}>Stein hoch</button>
        </div>

        <div style={row}>
          <button onClick={() => addItem('leaf')} style={btn}>Blatt</button>
          <button onClick={() => addItem('lantern')} style={btn}>Laterne</button>
          <button onClick={() => addItem('rake')} style={btn}>Rechen</button>
          <button onClick={() => addItem('waveRing')} style={btn}>Wellenkreis</button>
        </div>

        <div style={row}>
          <button onClick={clearRidges} style={btnDark2}>Rillen löschen</button>
          <button onClick={saveScene} style={btnGreen}>Speichern</button>
          <button onClick={loadScene} style={btnGreen}>Laden</button>
          <button onClick={exportPNG} style={btn}>PNG-Export</button>
          <button onClick={removeSelected} style={btnWarn} disabled={!selectedId}>Ausgewähltes löschen</button>
        </div>
        ...
        <div style={{ display:'grid', gap:8, marginTop:8 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            <button
              ...
              style={{ padding:'6px 10px', borderRadius:10, border:'1px solid rgba(0,0,0,0.15)', background: muted ? '#eee' : '#1b1b1b', color: muted?'#111':'#fff', cursor:'pointer' }}
            >
              {muted ? 'Unmute' : 'Mute'}
            </button>
        ...
        <div style={{ marginTop: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {(['morning','day','dusk','night'] as ThemeKey[]).map(t => (
              <button
                key={t}
                onClick={()=>{ setTheme(t); pushHistory(); soundRef.current?.clickUi() }}
      ...
      {/* Daily Zen Card (oben rechts) */}
      <DailyCard open={dailyOpen} onClose={() => setDailyOpen(false)} />
    </div>
  )
}

/* ========== Grain Texture Sprite (feines Korn) ========== */
